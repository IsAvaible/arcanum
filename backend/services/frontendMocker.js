...
async function testRequest(method, url, data = null) {
  try {
    ...
    console.log(`Request ${method.toUpperCase()} ${url} => Status: ${res.status}`, res.data);
    return res.data;
  } catch (error) {
    console.error(`Error in request ${method.toUpperCase()} ${url}`, error.response ? error.response.data : error.message);
  }
}
...
setTimeout(async () => {
  ...
  if (createChat && createChat.chatId) {
    ...
    const messageData = {content: "Hallo, wie geht es dir?", socketId: socket.id};
